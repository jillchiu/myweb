...
<?php
	...
	if(isset($check)) {
		...
		$re4 = $mysqli->query("select * from room where room_number='$room'");
		$ro3 = $re4->fetch_object();
		$rid = $ro3->rid;
		
		$re5 = $mysqli->query("select * from book where rid='$rid' order by check_in_date");
		$re7 = $mysqli->query("select * from book where rid='$rid' order by check_in_date");
...
<body>
	<div id="container">
		...
		?>
			...
			<table>
				<form method="post" action="booking_modify.php">
			...
			<form method="get" action="<?php print $_SERVER['PHP_SELF']; ?>"> 
				...
				<tr><td><FONT COLOR="F7FF00">Select to check the room status: </FONT></td>
					...
					<?php
						...
						while ($ro5 = $re6->fetch_object()){
							
							echo "<option value='".$ro5->room_number."'>".$ro5->room_number."</option>";
				...
				<?php 
					...
					if(isset($check)) { 
						...
						if(!$ro4 = mysqli_fetch_array($re5)){
							...
							while ($ro6 = $re7->fetch_object()){
								
								$output = "<FONT COLOR='F7FF00'>Room ".$room." has been booked at ".$ro6->check_in_date.".</FONT><br/>";
								print $output; 
				...
				*/
				...
				<br/>Click <a href="book_record.php"><FONT COLOR="FFFFFF">here</FONT></a> to back to front page.
		</div>
